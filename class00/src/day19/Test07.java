		...
		/*
		 	1. Pattern 클래스를 이용해서 검사하고 싶은 정규식 패턴을 만든다.
		 		...
		 		참고 ]
		 			이 클래스는 생성자 함수가 없으므로
		 			절대로 new시켜서 사용할 수 없고,
		 			더군다나 클래스의 속성이 더 이상 상속해줄 수 없는
		 			final속성이므로 상속해서 new시켜서 사용할 수도 없다.
		 			
		 			이런 경우는 소속 함수중 속성이 static이면서 반환값이 자기자신인
		 			함수를 찾아서 사용해야 한다.
		 */
...
//		Pattern pattern = Pattern.compile("0\\d{1,2}-\\d{3,4}-\\d{4}");
		...
		/*
		 	2. 패턴이 만들어졌으면 만들어진 패턴을 가지고 문자열을 검사하면 된다.
		 		검사 함수는 
		 			matcher()
		 		이 함수는 검사결과를 Matcher라는 클래스 타입으로 반환해준다.
		...
		// 이 때 사용하는 함수가 matches()이다. 반환값은 논리값으로 기억된다.
		/*
		String msg =""; //올바른||잘못된
		if(match.matches()) {
			msg = "올바른";
