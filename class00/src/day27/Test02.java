	...
	public void joinEvt() {
		btn1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// 카운트 값에 따라 적당히 처리해준다.
				if(cnt == 1) {
					// 이 때는 입력한 아이디를 사용하는 사람이 이미 있는 경우다.
					// 따라서 다시 입력받아야 한다.
					id.setText("");
					JOptionPane.showMessageDialog(null, "<html><h2 style=\"color:red\">"+sid+"는 사용불가능한 아이디 입니다.</h2></html>");
					return;
				} else {
				...
				System.out.println("Test02_데이터 꺼내온다");
				...
				mVO.setGen(sgen);
				mVO.setAvt(sno);
				System.out.println("Test02_Vo 담는다.");
				...
				if(cnt == 1) {
					JOptionPane.showMessageDialog(null, "<html><h2 style=\"color:red\">"+sid+"님 가입이 완료되었습니다.</h2></html>");
				} else {
					JOptionPane.showMessageDialog(null, "<html><h2 style=\"color:green\">"+sid+"는 가입이 취소되었습니다.</h2></html>");				}				
			}
			
		});
		System.out.println("Test02_mDao.addMemb(mVO)했다.");
	}
